...
static const unsigned char PP[] = {
    ...
    0xe9,
    0xe5, 0xea, 0xf0, 0x2a, 0xc6, 0x0a, 0xcc, 0x93, 0xed, 0x87};
                 ...
                 int w, int r, int b, void *key)
{
    ...
    else
    {
        ...
        if (vectors)
        { /* Print initial values of L and S */
            for (i = 0; i < l_words; i++)
            {
                printf("L[%3d] = ", i);
                pbuf((char *)L + i * n, n, 0);
...
void rc5_decrypt(void *rkey, int w, int r, void *ct, void *pt)
{
    ...
    {
        rot_amt = bits(A, n, lgw);
        sub(B, B, rk + 2 * i * n + n, n);
        rotl(B, B, w - rot_amt, n);
        eor(B, B, A, n);
        rot_amt = bits(B, n, lgw);
        sub(A, A, rk + 2 * i * n, n);
